...
namespace MazeGenerator
{
    ...
    /// 
    [RequireComponent(typeof(CellsBuilder))] 
    public class Generator : MonoBehaviour
    {
        [Header("Builder & ajustes")]
        ...
        [Header("Eventos")]
        [SerializeField] public UnityEvent _onMazeStarted   = new UnityEvent();
        [SerializeField] public UnityEvent _onMazeCompleted  = new UnityEvent();   

        [SerializeField] bool _debugView = true;
        List<CellsBuilder.Cell> _BuiltCells = new List<CellsBuilder.Cell>();


        void Awake()                                   // ← Se ejecuta antes que cualquier Start()
        {
            if (_cellsBuilder == null)
                _cellsBuilder = GetComponent<CellsBuilder>();

            if (_cellsBuilder == null)                 // doble verificación defensiva
                Debug.LogError("Generator: no se encontró CellsBuilder");
        }


        public void Generate()
        {
            if (_cellsBuilder == null)                 // evita llamada si sigue null
            {
                Debug.LogError("Generator.Generate: _cellsBuilder es null, abortando generación");
                return;
            }

            Debug.Log("Generator.Generate → inicia laberinto");

            _onMazeStarted.Invoke();
